...
def test_copilot_instructions_exists():
    ...
    assert (
        copilot_instructions_path.exists()
    ), "copilot-instructions.md should exist in repository root"
...
def test_copilot_instructions_content():
    ...
    assert (
        len(content.strip()) > 1000
    ), "copilot-instructions.md should contain substantial content"
